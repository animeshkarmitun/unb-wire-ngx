...
class ServiceConfig extends Component
{
    ...
    public string $wireName=''; public string $description=''; public bool $enabled=true;
    public function mount(string $service='en'): void { $this->service=$service;
        $s=Setting::where('key','service.'.$service)->first();
        if($s){ $v=$s->value; $this->wireName=$v['wireName']??''; $this->description=$v['description']??''; $this->enabled=$v['enabled']??true; }
    }
    ...
    public function render(){ return view('livewire.admin.service-config'); }
}
