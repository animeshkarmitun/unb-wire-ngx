...
class BillingService
{
    public function mrr(?string $period = null): float
    {
        $q = DB::table('client_packages')
            ...
            ->where(function ($w) {
                $w->whereNull('client_packages.ends_at')->orWhere('client_packages.ends_at', '>', now());
            });
    ...
    {
        $start = Carbon::parse($periodStart)->startOfMonth()->toDateString();
        $end = Carbon::parse($periodEnd)->startOfMonth()->endOfMonth()->toDateString();
        ...
        foreach ($clients as $row) {
            if (Invoice::where('client_id', $row->client_id)->whereDate('period_start', $start)->exists()) {
                continue;
            }
            $inv = Invoice::create([
