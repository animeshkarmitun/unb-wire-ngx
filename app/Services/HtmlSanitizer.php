...
class HtmlSanitizer
{
    ...
    public static function clean(?string $html): string
    {
        ...
        foreach ($xpath->query('//*') as $node) {
            ...
            if ($tag === 'div' && $node->parentNode instanceof \DOMDocument) {
                continue;
            }
            ...
            if ($node->hasAttributes()) {
                ...
                foreach ($node->attributes as $attr) {
                    ...
                    if (! in_array($name, self::ALLOWED_ATTR, true)) {
                        $toRemove[] = $name;
                    ...
                    if (in_array($name, ['href', 'src'], true)) {
                        ...
                        if (str_starts_with($v, 'javascript:') || str_starts_with($v, 'data:text/html') || str_starts_with($v, 'vbscript:')) {
                            $toRemove[] = $name;

                            continue;
                        }
                        if ($name === 'src' && ! preg_match('#^(https?://|/|data:image/(png|jpeg|gif|webp);base64,)#i', $val)) {
                            $toRemove[] = $name;
