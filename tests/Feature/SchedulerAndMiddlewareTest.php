...
class SchedulerAndMiddlewareTest extends TestCase
{
    ...
    {
        $cat = Category::factory()->create(); $user = User::factory()->create();
    ...
    {
        $client = \App\Models\Client::factory()->create(['status'=>'active']);
    ...
    public function test_presigned_url_service_aborts_without_config(): void
    {
        ...
        $user = User::factory()->create();
        $a = \App\Models\MediaAsset::create(['public_id'=>(string)\Illuminate\Support\Str::ulid(),'kind'=>'photo','status'=>'library','title'=>'t','caption'=>'c','credit_line'=>'UNB','mime'=>'image/jpeg','size_bytes'=>10,'checksum'=>hash('sha256','x'),'storage_disk'=>'s3','original_path'=>'missing.jpg','uploaded_by'=>$user->id]);
        try { app(\App\Services\Media\PresignedUrlService::class)->forAsset($a); $this->fail('should abort'); } catch (\Symfony\Component\HttpKernel\Exception\HttpException $e) { $this->assertEquals(500, $e->getStatusCode()); }
    }

    public function test_story_versions_immutable_via_model(): void
    {
        $cat = Category::factory()->create(); $user = User::factory()->create();
        $story = app(\App\Services\StoryService::class)->createDraft(['language'=>'en','headline'=>'orig','brief'=>'b','body_html'=>'<p>body long enough</p>','category_id'=>$cat->id], $user);
        app(\App\Services\StoryService::class)->updateDraft($story, ['headline'=>'v2'], 1, $user);
        $this->assertEquals(2, $story->refresh()->versions()->count());
    }

    public function test_download_ledger_increments(): void
    {
        $user = User::factory()->create(); $client = \App\Models\Client::factory()->create();
        $a = \App\Models\MediaAsset::create(['public_id'=>(string)\Illuminate\Support\Str::ulid(),'kind'=>'photo','status'=>'library','title'=>'dl','caption'=>'c','credit_line'=>'UNB','mime'=>'image/jpeg','size_bytes'=>100,'checksum'=>hash('sha256','y'),'storage_disk'=>'s3','original_path'=>'dl.jpg','uploaded_by'=>$user->id,'download_count'=>0]);
        app(\App\Services\Media\PresignedUrlService::class)->recordDownload($a, $client->id, null, 'original');
